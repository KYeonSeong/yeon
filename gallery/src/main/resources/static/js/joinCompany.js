$(document).ready(function() {
  ...
  $("#consentAll").click(function() {
    if($("#consentAll").is(":checked")) {
      $("input[name=companyConsent]").prop("checked", true);
    } else {
      $("input[name=companyConsent]").prop("checked", false);
    }
  });

  $("input[name=companyConsent]").click(function() {
    var total = $("input[name=companyConsent]").length;
    var checked = $("input[name=companyConsent]:checked").length;
  ...
  $(".companyPwd").on("keyup", function() {
    ...
    const conditionMessage = "비밀번호는 8자리 이상 영문/숫자/특수문자 조합으로 입력해주세요.";
  ...
  // 비밀번호 확인 체크
  $(".companyPwdCk").on("keyup", function() {
    const pwd = $(".companyPwd").val();
  ...
  $(".companyName").on("keyup", function() {
    const name = $(this).val();
    const namePattern = /^[a-zA-Z가-힣-0-9]+$/;
    if (!namePattern.test(name)) {
      $(".nameCondition").text("회사명은 한글,영문,숫자로만 입력 가능해요.").css("color", "red").css("font-size", "12px");
    } else {
      $(".nameCondition").empty();
    }
  });
      ...
      $.ajax({
        ...
        success: function(result) {
          if (result.exists) {
            ...
            $(".emailCondition").text("사용 가능한 이메일이예요.").css("color", "blue").css("font-size", "12px");
          }
        }
      });
    }
      ...
      $.ajax({
        ...
        success: function(result) {
          if (result.exists) {
            ...
            $(".telCondition").text("사용 가능한 전화번호예요.").css("color", "blue").css("font-size", "12px");
          }
        }
      });
    }
  ...
  $(".signUpCompany").on("click", function() {
    ...
    temp.querySelectorAll('input[type="text"], input[type="password"], input[type="email"]').forEach(input => {
      ...
      if ($(".emailCondition").text() === "이미 사용 중인 이메일이예요.") {
        errorField = $(".companyEmail");
        return false;
      }
      if ($(".telCondition").text() === "이미 사용 중인 전화번호예요.") {
        errorField = $(".companyTel");
        return false;
      }
    });
    ...
    if (!pwdPattern.test(pwd)) {
      $(".pwdCondition").text("비밀번호는 8자리 이상 영문/숫자/특수문자 조합으로 입력해주세요.");
      $(".companyPwd").focus();
      return;
    }
    
    // 비밀번호 확인 일치 여부 확인
    const pwdCk = $(".companyPwdCk").val();
    if (pwd !== pwdCk) {
  ...
  $(".emailSendBtn").on("click", function(){
    ...
    // 이메일 중복 여부 확인
    let findEmail = $(".companyEmail").val();
    if (!findEmail) {
      ...
      success : function(result) {
        if (result.exists) {
          ...
          $.ajax({
            ...
            url : "/emailSend",
            dataType : "json",
            data : {"emailSend" : $(".companyEmail").val()},
